...
const BookDetails = (props: BookDetails) => {
    ...
    useEffect(() => {
        GET(`/api/books/${id}`).then(data => setData(data))
    }, [id])
            ...
            <section className='row justify-content-center'>
                    <div className='col-12 col-md-6 card'>
                        <div className='card-body bg-light'>
                            ...
                            <Link to={`/books/${id}/update`} className='btn btn-outline btn-dark'>Update Book</Link>
                            <Link to='/books' className='btn btn-outline btn-primary mx-4'>Go Back to Books</Link>
                            <Link to='/books/new' className='btn btn-outline btn-dark'>Add Your Own!</Link>
                            </div>
